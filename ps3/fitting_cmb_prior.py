...
def lev_mar(f,g, x, y, err, guess, max_iter=100, rel_tol=1E-3):
    ...
    for i in range(max_iter):
        ...
        while increase:
            ...
            # N is the noise matrix with diagonal entries sigma^2 for uncorrelated noise
            # therefore, Ninv is the diagonal matrix of 1/sigma^2
            Ninv = np.diag(1.0/err**2)
            # lhs = J^T*N^-1*J + lambda*I
            lhs = grad.transpose() * Ninv * grad + lmfac * np.eye(grad.shape[1])
            # rhs = J^T * N^-1 * r
            rhs = grad.transpose() * Ninv * r
            # Calculate the change in parameters
            dp  = np.linalg.inv(lhs) * rhs
            # Squeeze as array to get back 1D array
...
def monte(params, nstep):
    ...
    for i in range(nstep):
        ...
        # Calculating the new chi-squared value, this time with the knowledge of a prior parameter
        n_chisq = chisqr(wmap[:,1],get_spectrum(ls, n_params),wmap[:,2]) + ((n_params[3] - 0.0544)/0.0073)**2
